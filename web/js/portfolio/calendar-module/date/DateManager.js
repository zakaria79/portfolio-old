export default class DateManager {
  ...
  getDateFirstDayInWeek(date) {
    ...
    let weekDay = this.getWeekDayNb(date.getDay()),
      dateFirstDayInWeek = new  Date(date.getFullYear(), date.getMonth(), date.getDate());
    dateFirstDayInWeek.setDate(dateFirstDayInWeek.getDate() - (weekDay - 1));
    return dateFirstDayInWeek;
  }

  getDatesForWeekOrDaysCalendar() {
    let calendar = this.app.cm.currentCalendar,
      i;
    calendar.today = false;
    for (i = 0; i <= calendar.dateArrayLength; i++) {
      calendar.dates[i] = this.setDate(this.add(calendar.currentDate, i));
      if (calendar.dates[i].dateStandard === this.app.dateToday.dateStandard) {
        calendar.today = true;
        calendar.todayDatesIndex = i + 1;
      }
    }
  }
  ...
  getStringDateAndHourFromDateObject(date) {
    return `${this.getStringFromOneDate(date)} à ${date.getHours()}:${date.getMinutes()}`;
  }
